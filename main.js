    ...
    if (dataAr[0] == 'sudo') {
      ...
      for (var i = 0; i < scList.length; i++) {
        if (dataAr[1] == scList[i]) {
          ...
          //create link entries
          try {
            var entry1 = linkMaker(
              shortcut[shortList][0][0],
              shortcut[shortList][0][1]
            )
          } catch (err) {
            var entry1 = ''
          }
          try {
            var entry2 = linkMaker(
              shortcut[shortList][1][0],
              shortcut[shortList][1][1]
            )
          } catch (err) {
            var entry2 = ''
          }
          try {
            var entry3 = linkMaker(
              shortcut[shortList][2][0],
              shortcut[shortList][2][1]
            )
          } catch (err) {
            var entry3 = ''
          }
          try {
            var entry4 = linkMaker(
              shortcut[shortList][3][0],
              shortcut[shortList][3][1]
            )
          } catch (err) {
            var entry4 = ''
          }
          try {
            var entry5 = linkMaker(
              shortcut[shortList][4][0],
              shortcut[shortList][4][1]
            )
          } catch (err) {
            var entry5 = ''
          }
          try {
            var entry6 = linkMaker(
              shortcut[shortList][5][0],
              shortcut[shortList][5][1]
            )
          } catch (err) {
            var entry6 = ''
          }
      ...
      window.open('http://google.com/search?q=' + data.replace('g.', ''))
    } else if (dataAr[0] == 'so') {
      window.open(
        'http://stackoverflow.com/search?q=' + data.replace('so.', '')
      )
      ...
      processBash(
        ...
        'ls :: list Shortcut lists',
        '. [filename] :: Open Shortcut lists',
        "nano [Term] :: Open local URI ('help' for all items)",
        'whoami :: Display UserAgent',
        'help- :: Page 2 of Help'
      )
    } else if (dataAr[0] == 'help-') {
      processBash('', '', '', '', '', '')
      ...
      if (dataAr[1] == 'help') {
        processBash(
          'settings :: Microsoft Settings',
          'steam :: Open Steam Game Library',
          '',
          '',
          '',
          ''
...
function linkMaker(nameC, linkC) {
  if (nameC != null && linkC != null) {
    return '<a tabindex="0" href=' + linkC + '>' + nameC + '</a>'
  } else {
    return ''
  }
}
...
window.onload = () => {
  ...
  //assign favourites
  document.getElementById('fav1').innerHTML = linkMaker(
    favourites[0][0],
    favourites[0][1]
  )
  document.getElementById('fav2').innerHTML = linkMaker(
    favourites[1][0],
    favourites[1][1]
  )
  document.getElementById('fav3').innerHTML = linkMaker(
    favourites[2][0],
    favourites[2][1]
  )
  document.getElementById('fav4').innerHTML = linkMaker(
    favourites[3][0],
    favourites[3][1]
  )
  document.getElementById('fav5').innerHTML = linkMaker(
    favourites[4][0],
    favourites[4][1]
  )
  document.getElementById('fav6').innerHTML = linkMaker(
    favourites[5][0],
    favourites[5][1]
  )
